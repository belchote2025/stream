    ...
    function initSearchAutocomplete() {
        ...
        searchInput.addEventListener('input', function() {
            ...
            timeout = setTimeout(async () => {
                try {
                    const response = await fetch(`/streaming-platform/api/content/popular.php?limit=5`);
                    ...
                    if (data.success && data.data) {
                        ...
                        if (results.length > 0) {
                            autocompleteContainer.innerHTML = results.map(item => `
                                <div class="autocomplete-item" onclick="window.location.href='/streaming-platform/content-detail.php?id=${item.id}'">
